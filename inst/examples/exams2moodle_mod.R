...

{
  function(x) {
    ...
    if(type == "cloze") {
      ...
      ## fraction of points given for partially correct solutions / alternative solutions
      ptfrac <- if(is.null(x$metainfo$exptfrac)) "100" else x$metainfo$exptfrac
      ptfrac <- if(!is.list(ptfrac)) {
        if(x$metainfo$type == "cloze") as.list(x$metainfo$exptfrac) else list(x$metainfo$exptfrac)
      } else x$metainfo$exptfrac
      ptfrac <- rep(ptfrac, length.out = n)
      ptfrac <- ifelse(ptfrac=="", "100", ptfrac)
      ...
      for(i in 1:n) {
        ...
        if(grepl("choice", x$metainfo$clozetype[i])) {
          ...
          if (x$metainfo$clozetype[i]=='mchoice') {
            # horizontal=MULTIRESPONSE_H or vertical=MULTIRESPONSE display of mchoice / multiresponse questions 
            if (is.null(x$metainfo$exmchoice_display[i]) | x$metainfo$exmchoice_display[i]=='') {
              # default to vertical
              mchoice_display <- 'MULTIRESPONSE'
            } else {
              mchoice_display <- x$metainfo$exmchoice_display[i]
            }
            tmp <- paste('{', points[i], ':', mchoice_display, ':', sep = '')
            # testing showed that Moodle does not allow any decimal places in the point fractions for this question type (!)
            p <- round(as.numeric(strsplit(ptfrac[[i]],'~')[[1]]), 0)
          } 
          if(k < 2) {
            tmp <- paste(ql, tmp)
            p <- paste('%', p, '%', sep = '')
        ...
        if(x$metainfo$clozetype[i] == "num") {
          for(j in 1:k) {
            splsol <- gsub(' ','',strsplit(solution[[i]][j],'~')[[1]])
            pfrac <- strsplit(ptfrac[[i]][j],'~')[[1]]
            stol <- strsplit(tol[[i]][j],'~')[[1]]
            #            msol <- paste('%', pfrac, '%', splsol,':', max(tol[[i]]), sep='', collapse='~')
            msol <- paste('%', pfrac, '%', splsol,':', stol, sep='', collapse='~')
            tmp <- c(tmp, paste(ql[j], ' {', points[i], ':NUMERICAL:', msol, if(numwidth) paste('~%0%', 
              fnums, ":0", sep = '') else NULL,
              '}', sep = ''))
          }
        }
        if(x$metainfo$clozetype[i] == "string") {
          for(j in 1:k) {
            splsol <- strsplit(solution[[i]][j],'~')[[1]]
            pfrac <- strsplit(ptfrac[[i]][j],'~')[[1]]
            msol <- paste('%', pfrac, '%', splsol, sep='', collapse='~')
            msol_lowercase <- paste('%', pfrac, '%', tolower(splsol), sep='', collapse='~')
            tmp <- c(tmp, paste(ql[j], ' {', points[i], ':SHORTANSWER:', msol,
              if(!usecase) paste('~', msol_lowercase, sep = '') else NULL,
              '}', sep = ''))
          }
        }
